...
source("R/level_1/figures_level1.R")
...
#model1 = file.path("stan_models", "m1_penalised_test.stan")
model2 = file.path("stan_models", "m2_penalised.stan")
...
for (country in countries) {
  ...
  save.image(file = paste0('outputs_level1/',country, '_TVEM.RData'))
}
...
for (country in countries) {
  ...
  if (!is.null(fig1)) {
    png(filename = paste0('figures/fit_beta/', country, '_beta.png'),
...
for (country in countries) {
  ...
  if (!is.null(fig2)) {
    png(filename = paste0('figures/fit_C/', country, '_contacts.png'),
        width = 1875, height = 1875)
    print(fig2)
    dev.off()
  }
}
